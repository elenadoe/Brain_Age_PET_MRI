...
data_path = '/DATA/doeringe/Dokumente/BrainAge/3_SUVR_new/'
output_csv = '/DATA/doeringe/Dokumente/BrainAge/3_SUVR_new/parcels.csv'
...
for sub in subj_list:
    ...
    foi = glob(data_path + "S*" + sub_name + "_" +  session + "*.nii")
    if foi:
...
# exclude data that could not be pre-processed (not all frames measured)
age = [age[x] for x in range(len(subj_list)) if subj_list[x] in subj_succ]
subj_list = [x for x in subj_list if x in subj_succ ]
