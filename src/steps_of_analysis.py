...
def cross_validate(df_train, col, models, model_names, model_params, splits,
                   scoring, rand_seed, y='age', info=False):
    """
    Cross-validation.
...
def brain_age(dir_mri_csv, dir_pet_csv, modality, rand_seed, atlas,
              y="age", database="ADNI", correct_with_CA=True,
              cv_outer=5, cv_inner=5,
    ...
    print(f"Considering {len(col_init)} ROIs before feature selection " +
          f"(Feature selection applied: {feat_sel}).")
