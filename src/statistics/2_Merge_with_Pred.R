...
pred_table_pet <- read.csv(sprintf(
  '2_BrainAge/PET_MRI_age0/results/ADNI/%s/PET-predicted_age_%s%s.csv',
...
pred_table_mri <- read.csv(sprintf(
  '2_BrainAge/PET_MRI_age0/results/ADNI/%s/MRI-predicted_age_%s%s.csv',
  group, group, model.add))
...
diagnoses <- read.csv(
  "2_BrainAge/PET_MRI_age0/data/ADNI/PsychPath/Diagnoses_upto2years.csv",
  na.strings = c("", "NA"))
...
write.csv(df, paste("2_BrainAge/PET_MRI_age0/data/ADNI/PsychPath/",
