...
get.data <- function(group){
  clin_data_bl <- read.csv2("2_BrainAge/PET_MRI_age/data/clin_data_bl.csv",
                            na.strings = "")
  diagnosis_24months <- read.csv2("2_BrainAge/PET_MRI_age/data/clin_data_24months3.csv",
                                  na.strings = "", dec = ".")
  diagnosis_earlier <- read.csv(paste(
    "2_BrainAge/PET_MRI_age/data/Diagnoses_upto2years_", group, "_2.csv", sep=""))
  ...
  diagnosis_24months <- subset(diagnosis_24months, select = c(PTID, DX, ABETA_24))
  
  clin_data <- merge(clin_data_bl, diagnosis_24months, by = "PTID", all.x = TRUE)
  
  model <- ifelse(group == "MCI", "_BAGGED", "")
  ...
  if (group == "CN") {
    ...
    df$Dementia_final <- ifelse(!is.na(df$DX), 
                            ifelse(df$DX == "Dementia",
                                ifelse(df$ABETA_24 < 1100 | df$ABETA_12 < 1100 |
                                         df$ABETA < 1100, "AD", 
                                  "Dementia"),
                                  df$DX),
                         ifelse(!is.na(df$DX12),
                            ifelse(df$DX12 == "Dementia",
                                ifelse(df$ABETA_12 < 1100 | df$ABETA < 1100, "AD",
                                 "Dementia"),
                                  df$DX12),
                         # no ABETA for month 6
                         ifelse(!is.na(df$DX6),
                                ifelse(df$DX6 == "Dementia",
                                       ifelse(df$ABETA < 1100, "AD",
                                "Dementia"),
                                df$DX6),
                         "NA")))
    # "036_S_4899" converts to dementia and back --> exclude
    df <- df[df$PTID != "036_S_4899",]
    table(df$DX_final)
...
predict.cognitive.impairment <- function(x) {
  ...
  traindata <- subset(x,
                      select=c(MRIBPAD, PETBPAD, AMY.cat, APOE4c, PTEDUCAT))
...
get.odds <- function(iter){
  # save significance
  coeff <- data.frame(s[[iter]]['coefficients'])
  write.csv(coeff, 
            paste("2_BrainAge/PET_MRI_age/results/conv/", 
                  group, "_CONV_",
                  as.character(whole.set), "model_", iter, "_NEW.csv", sep = ""))
  # plot and save odds ratios
  OR <- or_glm(data = samples[[iter]], model = models[[iter]]$finalModel, 
  ...
  g <- ggplot(OR, aes(x = oddsratio, y = predictor)) + 
    ...
    ylab("") +
    xlab("Odds ratio")
  ggsave(filename = paste(group, "_ODDS_CONV_compl_", iter, 
  ...
  write.csv(OR, 
            paste("2_BrainAge/PET_MRI_age/results/conv/", 
                  ...
                  iter, "_NEW.csv", sep = ""), row.names = F)
}
get.predictions <- function(iter){
  prediction.df <- data.frame(rowIndex = models[[iter]]$pred$rowIndex,
                              gt = models[[iter]]$pred$obs,
                              pred = models[[iter]]$pred$pred,
                              prob = models[[iter]]$pred$X1_decl)
  # add relevant variables to prediction df
  for (j in 1:nrow(prediction.df)){
    prediction.df$PTID[j] <- samples[[iter]]$PTID[prediction.df$rowIndex[j]]
    prediction.df$DX[j] <- samples[[iter]]$DX_final[prediction.df$rowIndex[j]]
    prediction.df$PETBPAD[j] <- samples[[iter]]$PETBPAD[prediction.df$rowIndex[j]]
    prediction.df$MRIBPAD[j] <- samples[[iter]]$MRIBPAD[prediction.df$rowIndex[j]]
    prediction.df$PETAGE[j] <- samples[[iter]]$PETAGE[prediction.df$rowIndex[j]]
    prediction.df$MRIAGE[j] <- samples[[iter]]$MRIAGE[prediction.df$rowIndex[j]]
    prediction.df$APOE[j] <- samples[[iter]]$APOE4[prediction.df$rowIndex[j]]
    prediction.df$APOEc[j] <- samples[[iter]]$APOE4c[prediction.df$rowIndex[j]]
    prediction.df$AMY.cat.c[j] <- samples[[iter]]$AMY.cat.c[prediction.df$rowIndex[j]]
  }
...
get.cutoff.general <- function(iter){
  # PLOTTING
  prediction.df <- predictions[[iter]]
  ...
  g <- ggplot(prediction.df, aes(x = bpad, y = prob)) +
    theme_classic() +
    geom_smooth(method = "glm", 
                method.args = list(family = "binomial"), 
                color = "gray", alpha = 0.2, se = F) +
    geom_point(aes(shape=gt, size=gt, alpha=gt, color = "gray",
                   fill = "gray")) +
    ...
    scale_fill_manual(guide = "none",
                      values = "gray") +
    ylab("Probability of Cognitive Decline") +
    xlab("BPAD") +
    theme(text = element_text(size=20))
  dens1 <- ggplot(prediction.df, aes(x = bpad, pattern = gt, fill = "gray",
                                     alpha = 0.5)) + 
  ...
  ggsave(filename = paste(group, "_", modality, "_CONV_compl", iter, 
                          as.character(whole.set), "_general.png", sep=""),
         path = "2_BrainAge/PET_MRI_age/results/conv/",
         width = 10, height = 10, device='tiff', dpi=300)
  detach(prediction.df)
}
get.cutoff.specific <- function(iter){
  # PLOTTING
  prediction.df <- predictions[[iter]]
  prediction.df <- prediction.df[!(prediction.df$AMY.cat.c=="NA"),]
  attach(prediction.df)
  ...
  g <- ggplot(prediction.df, aes(x = MRIBPAD, y = prob)) +
    theme_classic() +
    geom_smooth(method = "glm", 
                method.args = list(family = "binomial"), 
                aes(color = var_), alpha = 0.2, se = F, linetype = "dashed") +
    geom_point(aes(shape=gt, size=gt, alpha=gt, color = var_,
                   fill = var_)) +
    geom_hline(yintercept = 0.5, linetype = "dotted", col = "gray", size = 2) +
    annotate("text", x = min(MRIBPAD)+2, y = 0.5, col = "gray",
             label = "Cut-off", vjust = -0.5, size = 5) +
  ...
  ggsave(filename = paste(group, "_", modality, "_CONV_compl", iter, 
                          as.character(whole.set), "_specific.png", sep=""),
         path = "2_BrainAge/PET_MRI_age/results/conv/",
         width = 10, height = 10, device='tiff', dpi=300)
  detach(prediction.df)
}
...
# CHECK IF THERE IS A CORRELATION BETWEEN BAGs
cor.test(df.match1$PETBPAD, df.match1$MRIBPAD)
cor.test(df.match2$PETBPAD, df.match2$MRIBPAD)
t.test(df.match1$meanage[df.match1$DX.cat.c == "X0_stable"],
       df.match1$meanage[df.match1$DX.cat.c == "X1_decl"])
t.test(df.match2$meanage[df.match2$DX.cat.c == "X0_stable"],
       df.match2$meanage[df.match2$DX.cat.c == "X1_decl"])
table(df.match1$PTGENDER, df.match1$DX.cat.c)
table(df.match2$PTGENDER, df.match2$DX.cat.c)

# REPEAT ANALYSES WITH TWO DIFFERENT SETS OF STABLES
set.seed(0)
samples <- list(df.match1, df.match2)
models <- lapply(samples, predict.cognitive.impairment)
s <- lapply(models, summary)
s

iter <- c(1,2)
odds <- lapply(iter, get.odds)

# GET PREDICTIONS AND DERIVE CUT-OFFS
predictions <- lapply(iter, get.predictions)
cutoffs.general <- lapply(iter, get.cutoff.general)
cutoffs.specific <- lapply(iter, get.cutoff.specific)

# REPEAT CONVERSION ANALYSIS FOR MCI WITH DECL = AD
df <- get.data(group)
table(df$Dementia_final)
table(df$DX.cat.c)
...
# set.seed(0)
# models.sig <- lapply(list(df.match2), cognitive.impairment.sig)
